...
void OLED_Min_ShowChar(u8 x,u8 y,u8 chr)
{      	
		unsigned char c=0,i=0;	
		c=chr;//得到偏移后的值			
		if(x>Max_Column-1){x=0;y=y+2;}
		if(SIZE ==16)
			...
			OLED_WR_Byte(F8X16[c*16+i+8],OLED_DATA);
				...
				OLED_Set_Pos(x,y+1);
				for(i=0;i<2;i++)
				OLED_WR_Byte(x4y11[c][i],OLED_DATA);
...
{      			    
    ...
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(DzkCode[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Byte(DzkCode[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}
...
{ 	
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	...
 	GPIO_SetBits(GPIOC,GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11);	//PD3,PD6 输出高
